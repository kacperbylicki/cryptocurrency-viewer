...
(async (): Promise<void> => {
  const validationError = await Config.register({
    templates: [AppConfig, CacheConfig, ExternalApiConfig, DatabaseConfig],
  ...
  const app: INestMicroservice = await NestFactory.createMicroservice(
    AppModule,
    {
      ...
      options: {
        url: `${host}:${port}`,
        package: cryptocurrencyProtobufPackage,
        protoPath: `${protoPath}/proto/cryptocurrency.proto`,
      },
    },
  );
